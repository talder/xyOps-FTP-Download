...
### Fixed
- Fixed missing `.Size` property error when downloading a single file in all protocols (FTP, FTPS, SFTP)
- Single file download now correctly initializes file objects with `Size = 0` before download, actual size is captured after download completes
